...
- Phase D: 3D visualization and analysis tools.
- Phase E: attention-based communication for improved coordination.
- Phase F: high-fidelity physics simulation with PyBullet.
- Phase G: migration to Isaac Sim / Isaac Lab (planned).
- Phase H: sim-to-real and edge deployment preparation.
...
### Phase 5: Communication and Attention

1. Add attention mechanism to aggregate neighbor observations.
2. Enable policy to focus on relevant neighbors dynamically.
3. Compare performance against CTDE-only baseline.

Why:

- Attention improves coordination in dense swarms by learning which neighbors matter most.
- Scales better than fixed neighbor encodings for large N.

### Phase 6: High-Fidelity Physics Simulation

1. Implement PyBullet-based environment with rigid body dynamics.
2. Integrate gravity, drag, and collision physics.
3. Train policies in physics-realistic environment.
4. Compare sim-to-real transfer potential.

Why:

- Point-mass dynamics don't capture real quadrotor behavior.
- Physics simulation reveals control challenges before hardware testing.
- Prepares for future Isaac Sim migration.

### Phase 7: Isaac Sim Migration (Planned)

1. Migrate to Isaac Sim/Lab for GPU-accelerated physics.
2. Add visual sensors and domain randomization.
3. Test sim-to-real transfer readiness.

### Phase 8: Deployment and CV Packaging
...
Train with CTDE (centralized critic):

```powershell
python scripts/train_ctde.py --iterations 100 --num-drones 3
```

Train with CTDE + Attention:

```powershell
python scripts/train_attention.py --iterations 200 --num-workers 2
```

Train in PyBullet physics environment:

```powershell
python scripts/train_physics.py --iterations 200 --num-workers 2 --num-gpus 1
```

Visualize PyBullet simulation:
...
- ✅ CTDE with centralized critic implementation
- ✅ Attention mechanism for neighbor aggregation
- ✅ PyBullet physics environment with rigid body dynamics
- ✅ 3D visualization tools
- ✅ GPU support for training

**Planned:**

- Isaac Sim/Lab migration for GPU-accelerated physics
- Full quadrotor dynamics (currently simplified force control)
- Visual perception and sensor simulation
- Explicit communication protocols between agents
- Real hardware deployment

These are intentional incremental steps toward production-ready swarm control.
